 ...
 */
export function scaleHtml(html: string, scale: number): string {
 ...
 */
export function hasScalableQuantities(strings: string[]): boolean {
  return strings.some(s => /data-qty=/.test(s))
}

function buildDisplay(scaled: number, unit: string, originalDisplay: string): string {
  if (unit === 'count') {
